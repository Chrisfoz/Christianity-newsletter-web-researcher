...
def generate_html(openai_client, content, language, logo_path, author_name, company_name):
    ...
    response = openai_client.chat.completions.create(
        model="gpt-4o",
        messages=[
            {"role": "system", "content": f"You are a web developer and journalist specializing in creating beautiful, modern web pages using Tailwind CSS. Create a well-structured, informative HTML article in {language} based on the provided LLM news snippets. Use the given URLs as citations where appropriate. The page should use Tailwind styling, be responsive, and visually appealing. Focus on synthesizing the information coherently, highlighting key points, and providing insightful analysis within the context of the LLM field. Insert your content into the provided HTML template, replacing the comment '<!-- Article content will be inserted here -->'. Use appropriate HTML tags (h2, h3, p, a) for structuring the content, and ensure it matches the styling defined in the template."},
...
def save_html_page(html_content, folder, filename, author_name, company_name):
    os.makedirs(folder, exist_ok=True)
    filepath = os.path.join(folder, filename)

    # Check if the HTML content contains the required elements
    if (author_name not in html_content or
        company_name not in html_content or
        'data:image/png;base64,' not in html_content):
        raise ValueError("Generated HTML is missing required elements (logo, author name, or company name)")
...
def process_language(openai_client, content, lang, folder, logo_path, author_name, company_name):
    ...
    html_content = generate_html(openai_client, translated_article, lang.value, logo_path, author_name, company_name)
    save_html_page(html_content, folder, f"llm_news_article_{lang.value.lower()[:2]}.html", author_name, company_name)
